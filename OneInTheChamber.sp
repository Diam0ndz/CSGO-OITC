...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_SetPoints", SetPoints, ADMFLAG_ROOT, "Alias for sm_setpoints");
	RegAdminCmd("sm_Setpoints", SetPoints, ADMFLAG_ROOT, "Alias for sm_setpoints");
	RegAdminCmd("sm_SP", SetPoints, ADMFLAG_ROOT, "Alias for sm_setpoints");
	RegAdminCmd("sm_sp", SetPoints, ADMFLAG_ROOT, "Alias for sm_setpoints");
}
...
public Action SetPoints(int client, int args)
{
	...
	PrintToChatAll(" \x06%N \x0Bset \x06%N's points to \x06%.2f", client, target, pointNum);
	return Plugin_Handled;
}
...
stock void PlayerWon(int client)
{
	...
	if(Round < GetConVarInt(oitc_maxRounds))
	{
		...
		Round += 1;
		Points[client] = 0.0;
		PrintToChatAll(" \x0BCurrently starting round \x06%i out of round \x06%i", Round, GetConVarInt(oitc_maxRounds));
	}
	else
	{
		CS_TerminateRound(15.0, CSRoundEnd_Draw, true);
		GetNextMap(nextmap, sizeof(nextmap));
		PrintToChatAll(" \x0BChanging Map to\x06 %s \x0Bin 5 seconds...", nextmap);
		CreateTimer(5.0, Timer_SwitchMap);
	}
}
